...
async def lookup_business_type(name: str) -> str | None:
    ...
    try:
        resp = await claude_create(
            model=HAIKU_MODEL, max_tokens=20,
...
- Si el usuario manda una imagen con una lista de horarios/clases (ej: schedule semanal de un gym): 1) Extraé los horarios de la imagen. 2) Consultá el calendario para ver si ya existen esos eventos con horarios distintos. 3) Si hay diferencias, mostrá claramente cuáles cambian (ej: "Jueves: 17:00 → 18:00") y preguntá si quiere actualizar. 4) Si el usuario ya dijo explícitamente que quiere corregir/cambiar los horarios, confirmá los cambios que vas a hacer antes de ejecutarlos ("Voy a cambiar X, Y, Z — ¿confirmo?") y procedé al recibir el sí.
...
async def editar_reunion(text: str) -> tuple[bool, str]:
    response = await claude_create(
        model=SONNET_MODEL, max_tokens=250,
